...
class DrivingMemory:
    def __init__(self, encode_type='sce_language', db_path=None) -> None:
        ...
        if encode_type == 'sce_encode':
            ...
            if os.environ["OPENAI_API_TYPE"] == 'azure':
                ...
                # 创建一个OpenAI文本向量嵌入模型对象，用于把场景描述等文本转成向量
                self.embedding = OpenAIEmbeddings()
            else:
                raise ValueError(
    ...
    def retriveMemory(self, driving_scenario: EnvScenario, frame_id: int, top_k: int = 5):
        if self.encode_type == 'sce_encode':
            ...
            query_scenario = driving_scenario.describe(frame_id) # 获取现在场景
            similarity_results = self.scenario_memory.similarity_search_with_score(
                query_scenario, k=top_k)  # 检索最相似的top_k个记忆
            fewshot_results = []
    ...
    # 增加记忆
    def addMemory(self, sce_descrip: str, human_question: str, response: str, 
                  action: int, sce: EnvScenario = None, comments: str = ""):
        if self.encode_type == 'sce_encode':
            pass
        elif self.encode_type == 'sce_language':
